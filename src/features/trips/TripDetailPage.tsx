...
type TabId = Category | `person-${string}`;
...
export function TripDetailPage() {
  ...
  // Build tabs: categories that have items + person tabs for kleren
  ...
  // Tab definitions
  const tabs: { id: TabId; label: string; count: number; checked: number }[] = [];
  ...
  for (const person of persons) {
    ...
    if (arr && arr.length > 0) {
      tabs.push({ id: `person-${person.id}`, label: person.name, count: arr.length, checked: arr.filter(i => i.checked).length });
    }
  }

  // Ensure activeTab is valid
  const validTab = tabs.find(t => t.id === activeTab) ? activeTab : (tabs[0]?.id ?? 'stuff');
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Category tabs — scrollable like Excel sheet tabs */}
      <div className="flex items-center gap-1 overflow-x-auto pb-1 -mx-4 px-4 scrollbar-hide">
        {tabs.map(t => (
          ...
          >
            {t.label}
            <span className={`ml-1 text-xs num ${validTab === t.id ? 'text-paper/70' : 'text-muted'}`}>
              {t.checked}/{t.count}
            </span>
          </button>
        ))}
